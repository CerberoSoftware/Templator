...
const ICONS: Record<string, string> = {
  X: `<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="currentColor"><path d="M18.244 2.25h3.308l-7.227 8.26 8.502 11.24H16.17l-4.714-6.231-5.401 6.231H2.746l7.73-8.835L1.254 2.25H8.08l4.261 5.636zm-1.161 17.52h1.833L7.084 4.126H5.117z"/></svg>`,
...
/**
 * Extract the `d` attribute of the first <path> in an SVG string.
 * Used to identify a platform from its icon when labels are hidden.
 */
function svgPathD(svgString: string): string {
  const m = / d="([^"]+)"/.exec(svgString)
  return m ? m[1] : ''
}

/**
 * Build a lookup map from SVG path `d` value → platform name.
 * Computed once on first use.
 */
let _iconPathMap: Map<string, string> | null = null
function iconPathMap(): Map<string, string> {
  if (_iconPathMap) return _iconPathMap
  _iconPathMap = new Map()
  for (const [platform, svg] of Object.entries(ICONS)) {
    const d = svgPathD(svg)
    if (d) _iconPathMap.set(d, platform)
  }
  return _iconPathMap
}

/**
 * Attempt to identify a social platform by matching the SVG <path d="..."> in
 * the rendered icon against the known ICONS map. Returns the platform name or
 * 'X' as a safe fallback.
 */
export function platformFromSvg(svgEl: SVGElement | Element): string {
  const path = svgEl.querySelector('path')
  if (!path) return 'X'
  const d = path.getAttribute('d') ?? ''
  return iconPathMap().get(d) ?? 'X'
}
...
export function socialIcon(platform: string, size: number, color: string): string {
  const svg = ICONS[platform] ?? ICONS['X']
  return svg.replace(
    '<svg ',
  ...
  parse: (tr) => {
    ...
    if (isRow) {
      directDivs[0].querySelectorAll<HTMLElement>(':scope > span[style*="inline-block"]').forEach((span) => {
        ...
        // Fix: when showLabels is false there is no label <span>, so recover
        // the platform name from the SVG icon path instead of defaulting to
        // 'X' for every entry.
        const labelSpan = span.querySelector<HTMLElement>('span[style*="font-family"]')
        const svgEl = span.querySelector('svg')
        const platform = labelSpan?.textContent?.trim() || (svgEl ? platformFromSvg(svgEl) : 'X')
      ...
      directDivs.forEach((div) => {
        ...
        const href = a?.getAttribute('href') ?? ''
        const labelSpan = div.querySelector<HTMLElement>('span[style*="font-family"]')
