 ...
 * 'Twitter' as a safe fallback.
 */
export function platformFromSvg(svgEl: SVGElement | Element): string {
  const path = svgEl.querySelector('path')
  if (!path) return 'Twitter'
  const d = path.getAttribute('d') ?? ''
  return iconPathMap().get(d) ?? 'Twitter'
}
